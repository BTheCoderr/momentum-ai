...
const InsightsScreen = ({ navigation }: any) => {
  ...
  const handleSaveInsight = async (insight: any) => {
    try {
      ...
      Alert.alert('Success', 'Insight saved to your favorites!');
    } catch (error) {
  ...
  const handleShareInsight = async (insight: any) => {
    try {
      const shareMessage = `💡 Insight from Momentum AI:\n\n${insight.title}\n\n${insight.description}`;
      ...
      if (result.action === Share.sharedAction) {
        analytics.track('insight_shared', {
  ...
  const handleAICoach = (insight: any) => {
    navigation.navigate('Chat', {
      initialPrompt: `Let's discuss this insight: "${insight.title}". ${insight.description} How can I apply this to improve my progress?`,
      context: {
